...
$oas = new SoapOAS($settingCache->getData("CWPS_Address")); //初始化OAS客户端
$oas->setTransactionAccessKey($settingCache->getData("CWPS_TransactionAccessKey")); //设置CWPS访问密码
...
switch($action) {
	...
	case 'add_submit':
		if(empty($IN[GroupID])) showmessage('', "{$base_url}o=access_group::view");
	...
	case 'edit':
		if(empty($IN[AccessID])) showmessage('', "{$base_url}o=access_group::view");
