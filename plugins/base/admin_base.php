...
switch($action) {
	...
	case 'commentSearchAdmin':
		...
		if(!empty($IN['start_time'])) {
			$where = " CreationDate > ".strtotime($IN['start_time'])." AND CreationDate < ".(strtotime($IN['end_time']) + 3600*24);
	
		} elseif(!empty($IN['viewmode'])) {
			if($IN['viewmode']=='approved')  $where = " Approved=1";
			elseif($IN['viewmode']=='unapproved')  $where = " Approved=0";
			else $where = " Approved=1 || Approved=0 ";
		} else {
	...
	case 'commentDel':
		if(!empty($IN['CommentIDs'])) {
			foreach($IN['CommentIDs'] as $key=>$var) {
				if(!empty($var))	$result =  $base->commentDel($var);
			}
			if($result) 
	...
	case 'commentApprove':
		if(!empty($IN['CommentIDs'])) {
			foreach($IN['CommentIDs'] as $key=>$var) {
				if(!empty($var))	$result =  $base->commentApprove($var);
			}
			if($result) 
