...
//暂时关掉此功能,没什么意义,反而增加设置复杂度
        //检查在后台搜索插件中设置的可搜索字段是否匹配
        $validFields = explode(',', $Setting['AllowSearchField']);
		if(empty($Setting['AllowSearchField'])) {}
		elseif(!in_array($IN['Field'], $validFields)) goback('invalid_field');



     //处理 搜索字段 和 关键字 数组,组合成条件串 同时 形成逗号分隔串供URL参数传递用(add by easyt 2005.02.24
	...
	$andor_list = '';	//逻辑运算符列表
        foreach($IN['Field'] as $key=>$var) {
            if ( !empty($IN['Field'][$key]) && !empty($IN['Keywords'][$key]) ) {
                if( $field_where=='' ) {
                    $field_where = $IN['Field'][$key]." LIKE '%".str_replace(" ", "%", $IN['Keywords'][$key])."%'";   //组合,关键字中间的空格变成%
					...
					if ( $isAndor && (in_array(strtoupper($IN['Andor'][$key-1]), array('AND','OR'))) ) {
						...
						$field_where .= " ".$IN['Andor'][$key-1]." ".$IN['Field'][$key]." LIKE '%".str_replace(" ", "%", $IN['Keywords'][$key])."%'";    //组合,关键字中间的空格变成%
					}
					else {
						$field_where .= ' AND '.$IN['Field'][$key]." LIKE '%".str_replace(" ", "%", $IN['Keywords'][$key])."%'";    //组合,关键字中间的空格变成%
					}
					$field_list .= ','.$IN['Field'][$key];
					$keywords_list .= ','.$IN['Keywords'][$key];
                }
