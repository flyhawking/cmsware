...
class kTemplate {
    ...
    var $cache_dir       =  'cache';
	var $lang_dir = '';
	var $compile_lang = true; // whether to compile language label in the template.
	...
	var $cache_use_sub_dirs = false; //缓存使用分卷目录
	var $add_meta_mark = true; //是否添加CMSware标识到Meta标记

	function kTemplate($params = NULL)
	{
		if (isset($params['template_dir'])) $this->template_dir = $params['template_dir']; 
		else $this->template_dir = SYS_PATH.'skin/admin/';


		if (isset($params['compile_dir'])) $this->compile_dir = $params['compile_dir'];
		else $this->compile_dir = CACHE_DIR.'templates_c/';


		if (isset($params['cache_dir'])) $this->cache_dir = $params['cache_dir'];
		else  $this->cache_dir = CACHE_DIR.'cache/';

		if (isset($params['lang_dir'])) $this->cache_dir = $params['lang_dir'];
	}
    ...
    {	
		...
		if($kTemplate_compiler->compile($file_name, $this->compilefile_prefix . $this->format($this->template_name)))	return true;
		else return false;
	...
	{		
			...
			ob_end_clean();	
			if($this->enableMark) {
				$contents = empty($GLOBALS['SYS_ENV']['CMSware_Mark']) ? $contents : $contents.$GLOBALS['SYS_ENV']['CMSware_Mark'];
			}
			$contents = empty($GLOBALS['SYS_ENV']['CMSware_Powered']) ? $contents : preg_replace("'(<title>)(.*)(</title>)'isU", "\\1\\2".$GLOBALS['SYS_ENV']['CMSware_Powered']."\\3", $contents );
	...
	{	
		//$this->cache_dir. md5($_tplname . $_cacheid) . '.cache';
 	...
 	function fetch($file_name, $compile = 0) 
	{
		...
		if(is_object($Error)) {
			...
			if(is_object($TemplateError)) $TemplateError->setErrorHander();
		}
		...
		if(empty($this->lang_dir)) $this->lang_dir =  $this->template_dir;
		$this->lang_name = $this->lang_dir . $file_name .'.php';

		if($this->forceCompile) {

 			if($this->_compile($this->template_name))	$contents =  $this->_fetch($this->compile_name, $compile);

		} elseif($this->checkTplModify) {
		...
		if(is_object($Error)) $Error->setErrorHander();
 	...
 	function display($file_name, $enable_gzip = NULL)
	{
		...
		if(!empty($enable_gzip) || $GLOBALS['SYS_ENV']['enable_gzip']) {
