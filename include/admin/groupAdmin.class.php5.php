...
class groupAdmin extends iData {
	...
	function getInfo($gId)
	{
		...
		//$result[gPubAuth] = '[,'.$result[gPubAuth].',]';
		...
		switch($mode) {
			case 'Read':
				if($gId == '') {
					...
					if($gInfo['canMakeG'] == 1) return true;
					...
					if($gInfo['CreationUserID'] == $sys->session['sUId']) return true;

					if($gInfo['gId'] == $sys->session['sGId']) return true;
