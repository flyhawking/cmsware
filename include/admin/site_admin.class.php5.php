...
class site_admin extends iData {
	...
	function restore($NodeID, $targetNodeID)
	{
		...
		//$sql2 = "UPDATE $table->content_index SET `State`= 0 WHERE NodeID IN($SubNodeID)";
	...
	function isValid($add=0)
	{
		...
		if($result[nr]+$add > $license_array['Node-num'] && $license_array['Node-num']!=0) {
	...
	function getAll4Tree($ParentID = 0)
	{
		...
		while(!$result->EOF) {
			...
			//$data[$i]['Name_NodeID'] = $result->fields['Name']."(".$result->fields['NodeID'].")";
			$i++;
			$result->MoveNext();
	...
	function isSiteAdmin()
	{
		...
		if($sys->Auth[canNode]) return true;  
	 ...
	 * 访问权限检测
	 * 
	 * $mode: Read,Write,Manage,Approve,Publish
	 * 
	 */
	function canAccess(&$NodeInfo, $mode) 
	{
		...
		if($sys->session['sGIsAdmin'] == 1) return true;
		if($sys->Auth[canNode]) return true; //用户组处可以另外分配整个站点管理模块的权限
		if($this->isCreationUser($NodeInfo)) return true;
		...
		if(!empty($NodeInfo['Permission'.$mode.'G'])) $posG = strpos(','.$NodeInfo['Permission'.$mode.'G'].',', ','.$sys->session['sGId'].',');
		else $posG = false;

 
		if($posG === false) {
			 
			if(!empty($NodeInfo['Permission'.$mode.'U'])) $posU = strpos(','.$NodeInfo['Permission'.$mode.'U'].',', ','.$sys->session['sUId'].',');
			else return false;

			
			if($posU === false) return false;
			else return true;
	...
	function isCreationUser(&$NodeInfo)
	{
		...
		if($NodeInfo['CreationUserID'] == $sys->session['sUId']) return true;
		else return false;
	}
	...
	//{{{ add node property admin start
	function getAllFieldsInfo()
	{
		global $table,$db,$NODE_FIELDS_INFO;
	...
	function addField($data)
	{
		...
		$sql = "ALTER TABLE $table->site ADD `{$data[FieldName]}` {$data[FieldType]} $length  NOT NULL";
		if($db->query($sql)) {
	...
	function editField($FieldID, $data)
	{
		...
		$sql = "ALTER TABLE $table->site CHANGE `{$fieldInfo[FieldName]}` `{$data[FieldName]}` {$data[FieldType]} $length  NOT NULL";
