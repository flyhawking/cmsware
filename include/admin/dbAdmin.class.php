...
class adminDatabase{
	...
	{
		global $db,$table,$db_config,$TableStats;
		...
		while(!$recordSet->EOF) {
			if(preg_match($pattern,$recordSet->fields[Name])) {
				...
				if($result['Data_length'] > 1048576) {
 				...
 				if($result['Index_length'] > 1048576) {
...
/*
BackupIndex_2005_03_01_123456.xml

<Backup>
<file>CMS_backup_2005_03_01_123456.php</file>
<file>CMS_backup_2005_03_01_123456_1.php</file>
<file>CMS_backup_2005_03_01_123456_2.php</file>
<file>CMS_backup_2005_03_01_123456_3.php</file>
<file>CMS_backup_2005_03_01_123456_4.php</file>
</Backup>
*/	
	function backupInit($tablename, $drop)
	{
		global $table,$db,$_SESSION,$db_config,$SYS_ENV;
		...
		if (count($tablename) == 0) {
			...
			foreach($tablename AS $k=>$v) {
				if(strpos($v, 'admin_sessions')) continue;
	...
	function dumptable($table_name, $start = 0, $offset = 0)  //from vbb
	{ 
		global $db;
		$tabledump .= "";
		$db->SetFetchMode('num');
		...
		//print_r($rows);
		if(!$rows->EOF) 	$numfields=$rows->FieldCount();
	...
	{		
			...
			while($file=$dir->read()) {
				if( $file=="." || $file=="..") {
					continue;
				} elseif( is_dir($SYS_ENV[backupPath]. $file)) {
					continue;
							
				}else {
					if(preg_match("/CMS_backup_(.*).xml/isU", $file)) {
						$temp = explode('.', $file);
						$list[] = array(
							'filename'=>$file,
							'name'=> $temp[0],
