...
class workflowAdmin extends iData {
	...
	{
		global $table;
		if($this->isStateExists($this->insData[State]))  return false;
		elseif($this->insData[State] < 100) {
			 return false;
		} else {
			if($this->dataInsert($table->workflow_state))
				return true;
			else return false;		
	...
	function update_state($ID)
	{
		...
		if($this->isStateExists($this->insData[State]) && $StateInfo[State] != $this->insData[State])  return false;
		elseif($this->insData[State] < 100) {
			 return false;
		} else {	
			$where="where ID=".$ID;
	...
	function getAllStateByGroup($wID, $gId)
	{
		...
		$i = 0;
		while(!$result->EOF) {
			if($i == 0)	$data = $result->fields[StateBeforeOp];
			else {
