...
class tpl_cate_admin extends iData {
	...
	function canAccess(&$CateInfo, $mode) 
	{
		...
		if($this->isCreationUser($CateInfo)) return true;
		...
		if(!empty($CateInfo[$mode.'G'])) $posG = strpos(','.$CateInfo[$mode.'G'].',', ','.$sys->session['sGId'].',');
		else $posG = false;
		 
		if($posG === false) {
			 
			if(!empty($CateInfo[$mode.'U'])) $posU = strpos(','.$CateInfo[$mode.'U'].',', ','.$sys->session['sUId'].',');
			else return false;
	...
	function isCreationUser(&$CateInfo)
	{
		...
		if($CateInfo['CreationUserID'] == $sys->session['sUId']) return true;
		else return false;
	}
